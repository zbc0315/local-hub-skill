...
def test_offline_search_filters_header_lines(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str],
) -> None:
    """Offline search must not emit the `AUTO-GENERATED` comment or `# Data Hub Index` header."""
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("HUB_ROOT", "jim@host:/srv")
    monkeypatch.delenv("HUB_REMOTE_DISPATCH", raising=False)
    cached = (
        ...
        "|---|---|---|---|---|\n"
        "| covid-jhu | COVID | timeseries | daily data | datasets/covid-jhu |\n"
    )
    write_cached_index("jim@host:/srv", cached)

    monkeypatch.setattr(sys, "argv", ["hub", "search", "data"])
    fake = MagicMock()
    fake.returncode = 255
    fake.stdout = b""
    fake.stderr = b"ssh: connect error\n"
    ...
    captured = capsys.readouterr()
    # Hits the covid row, but not the header comment.
    assert "covid-jhu" in captured.out
    assert "AUTO-GENERATED" not in captured.out
    assert "# Data Hub Index" not in captured.out
